...
ListLayout
  ...
  template(v-for="payment in payments", :slot="'row-' + payment.id")
    ...
    td.crud__cell
      p.crud__state.crud__state_detail(
        :class="{ 'state-warning': [0, 1].indexOf(payment.status) !== -1 }"
      ) {{ statuses[payment.status] }}
  template(v-for="payment in payments", :slot="payment.id + '-actions'")
    button(
      v-if="isCancelable(payment) && !canceling[payment.id]",
      @click="$set(canceling, payment.id, 'confirm')"
    ) Cancelar la compra
